...
```{r}
modelOld <- modelSwarm(datOld)
modelNew <- modelSwarm(datNew)
```
...
dataFromRefFiles <- function(dataset) {
  tibble(file = Sys.glob(str_c("../data/swarm-", dataset, "/references/*"))) |>
...
#| fig-width: 8
#| fig-height: 5
#| fig-cap: Upload times in the April (left panel) and June (right panel) benchmarking experiments.
#| label: fig-uploads
