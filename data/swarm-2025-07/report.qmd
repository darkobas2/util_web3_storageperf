...
We can visually compare download times across the 2.5 and 2.6 benchmarking runs. We do this both for the `DATA` (@fig-compare-data) and `RACE` (@fig-compare-race) retrieval strategies. For smaller file sizes (especially in the absence of erasure coding), the new release may appear to be slightly faster, but for large files it looks clearly slower. We can also display the same data by aggregating information across servers, using the panel columns to distinguish between retrieval strategies and thus needing just one plot for all the data (@fig-compare-aggr). This aggregated plot is still consistent with the above intuitions.
...
#| fig-width: 8
#| fig-height: 9
#| fig-cap: As @fig-compare-data, but for the `RACE` retrieval strategy. The first panel row (no erasure coding) is the same as the first panel row in @fig-compare-data, since the `DATA`/`RACE` distinction only applies when downloading erasure-coded content.
...
#| fig-width: 8
#| fig-height: 9
#| fig-cap: Combining the data of @fig-compare-data (first panel column) and @fig-compare-race (second panel column), by aggregating data points across the three servers in each group.
#| label: fig-compare-aggr
...
#| fig-width: 8
#| fig-height: 9
#| fig-cap: As @fig-compare-race, but with $z$-scores along the y-axis.
#| label: fig-z-race
...
#| fig-width: 8
#| fig-height: 9
#| fig-cap: Combining the data of @fig-z-data (first panel column) and @fig-z-race (second panel column), by aggregating data points across the three servers in each group.
#| label: fig-z-aggr
...
downloadDat |>
  ...
  mutate(adv = fct_relevel(adv, "Release 2.5", "No difference")) |>
  mutate(strategy = ifelse(strategy == "RACE", "RACE", "NONE/DATA")) |>
...
We can also check how much advantage the `RACE` strategy offers over `DATA`, for each file size category. The results are qualitatively similar for the 2.5 release (@fig-erasure-box-pre) as for the 2.6 release (@fig-erasure-box-post) runs, except for 50 MB files. Here the 2.5 release sometimes gave a speed advantage to the `RACE` strategy. In the 2.6 release, `RACE` appears to be inferior for 50 MB files.
...
downloadDat |>
  ...
  mutate(strategy = as_factor(strategy)) |>
  mutate(size = as_factor(str_c("Size: ", size))) |>
  # Remove outliers (so they don't distort the plot):
  filter(time_sec < 1.5*IQR(time_sec) + quantile(time_sec, 0.75),
         .by = c(size, strategy, erasure)) |>
  ggplot(aes(x = erasure, y = time_sec, color = strategy)) +
  geom_quasirandom(alpha = 0.3, dodge.width = 0.6) +
...
A comparison of the raw upload times from Releases 2.5 and 2.6 is shown in @fig-upload-raw. Unlike for download times, it appears clear that Release 2.6 has faster upload times in most file size and erasure coding categories. This impression is further reinforced by looking at the $z$-scores instead of the raw upload times (@fig-upload-z).
...
Like in the case of download times, we can make a category-by-category comparison of corresponding upload times across the two experiments, using non-parametric Mann--Whitney tests (and correcting $p$-values for multiple comparisons using the false discovery rate method). The results are in @fig-wilcox-upload. Indeed, in most cases the new release has clearly faster uploads, and substantially so for larger (10 MB and 50 MB) files. The differences here are large enough to contribute positively to user experience.

```{r}
#| out-width: 90%
#| fig-width: 7
#| fig-height: 8.5
#| fig-cap: Group-by-group comparison of results from the Release 2.5 and Release 2.6 upload time data, using Mann--Whitney tests. The y-axis shows the estimated difference (point) plus/minus 95% confidence intervals (error bars) from a Mann--Whitney test applied to each distinct file size / retrieval strategy / erasure coding combination. Results favoring the new 2.6 release with $p < 0.05$ (after false discovery rate correction) are in yellow, and those with $p > 0.05$ (non-significant results) are in grey. (Those favoring the old 2.5 release would be in blue, but there aren't any such results here.)
